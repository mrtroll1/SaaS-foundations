...
from allauth.account.signals import (
    user_signed_up as aullauth_user_signed_up,
...
class Customer(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.stripe_id or not self.stripe_id.startswith('cus_'):
...
def aullauth_user_signed_up_handler(request, user, *args, **kwargs):
    email = user.email
    Customer.objects.create(
        user=user, init_email=email,
        init_email_confirmed=False
    )

aullauth_user_signed_up.connect(aullauth_user_signed_up_handler)
